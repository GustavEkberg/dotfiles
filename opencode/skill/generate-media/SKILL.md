...
**Catalog last audited:** 2026-08-08 against the English entries in `https://docs.kie.ai/sitemap.xml`.

**Verified model IDs and doc paths live in `references/kie-models.md`** — 127 market
models, each ID read from its live OpenAPI schema. Read that file before searching
the sitemap; it saves a fetch and avoids guessing at kie's irregular doc routes.
...
| Higher-tier Wan 2.7 image output | Wan 2.7 Image Pro | `wan/2-7-image-pro` |
| Newer Qwen generation tier | Qwen 3 | `qwen3/text-to-image`, `qwen3/pro-text-to-image` |
| Lightweight general image generation | Z-Image | `z-image` |
...
| Split an image into editable layers | Seedream 5 Pro Layer Decomposition | Market (`seedream/5-pro-layer-decomposition`) |
| Prompt-guided edit / remix of an Ideogram image | Ideogram 3.0 Edit / Remix | Market (`ideogram/v3-edit`, `ideogram/v3-remix`) |
| Edit or remix a consistent character | Ideogram Character Edit / Remix | Market (`ideogram/character-edit`, `ideogram/character-remix`) |
| Newer Qwen editing tier | Qwen 3 I2I | Market (`qwen3/image-to-image`, `qwen3/pro-image-to-image`) |
...
| Newer Seedance tier | Bytedance Seedance 2.5 | Market (`bytedance/seedance-2-5`) |
...
| Audio-driven talking video | InfiniTalk | Market (`infinitalk/from-audio`) |
| Character-conditioned or audio-conditioned Gemini Omni output | Gemini Omni Character / Audio | Market (`gemini-omni-character`, `gemini-omni-audio`) |
...
| Extend a PixVerse clip | PixVerse V6 Extend | Market (`pixverse-v6/extend`) |
| Transition between two frames | PixVerse V6 Transition | Market (`pixverse-v6/transition`) |
| Edit a HappyHorse clip | HappyHorse Video Edit | Market (`happyhorse/video-edit`) |
| Lip-sync an existing video to new audio | Volcengine Lip Sync | Market (`volcengine/video-to-video-lip-sync`) |
...
| Broad language coverage for narration | ElevenLabs Multilingual v2 | Market (`elevenlabs/text-to-speech-multilingual-v2`) |
...
**Do not derive a documentation URL from a model ID.** Kie documentation routes are irregular and change independently of request model IDs — roughly a third of market models have a doc path that differs from their model ID. For example, `flux-2/flex-text-to-image` is documented under `/market/flux2/flex-text-to-image`, `nano-banana-pro` under `/market/google/pro-image-to-image`, and `kling-3.0/motion-control` under `/market/kling/motion-control-v3`.

1. Look up the model in `references/kie-models.md`. It maps every known model ID to its verified doc path.
2. If it is not there (or the lookup 404s, meaning the catalog moved), fall back to the sitemap: fetch `https://docs.kie.ai/sitemap.xml`, ignore `/cn/` and `/cnmarket/` paths unless the user requests Chinese docs, and search the English URLs for the provider/model name plus the operation (text-to-image, image-to-video, edit, extend, and so on).
3. Fetch the exact matching page and read its OpenAPI specification.
4. Copy the request `model` enum/default from the OpenAPI schema. Never infer it from the page URL or this skill's curated tables.
5. **Cross-check the enum against the description sentence** on the same page. Some pages carry a stale `enum`/`default` while the prose "Must be `X` for this endpoint" names the correct ID. When they disagree, use the prose ID and fall back to the enum on a 422. Known cases are listed in `references/kie-models.md`.
...
`references/kie-models.md` is the verified snapshot; the official sitemap is the live inventory:

```
https://docs.kie.ai/sitemap.xml
```

Search it whenever the reference file has no match, rather than treating this skill's curated tables as exhaustive. The sitemap also carries chat/LLM entries under `/market/chat/`, `/market/claude/`, `/market/gemini/`, `/market/grok/`, and `/market/codex/` — those are text models, out of scope here. Then fetch the exact English model page and use its OpenAPI `model` enum/default.
...
If the available web fetch tool or `curl` cannot retrieve docs.kie.ai (for example because of rate limiting or network issues), use whatever browser automation tooling is available in the current agent (for example a browser MCP server) to browse the docs interactively.
