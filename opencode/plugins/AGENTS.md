...
Local plugins in `~/.config/opencode/plugins/` are **not** autoloaded. Reference explicitly in `opencode.json`:
...
Anthropic started fingerprinting tool name casing after the `mcp_` prefix. Lowercase names like `mcp_bash` and `mcp_read` trigger the same misleading 400 rejection.
